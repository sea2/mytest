...
/**
 * Created by yejingjie on 2016/1/4.
 */
...
public class NetworkUtils {
    ...
    public static String getMacAddress(Context context) {
        ...
        try {
            ...
            if (StringUtil.isNotEmpty(localMac) && StringUtil.isExistSubString(localMac, ":") && !StringUtil.isEquals("02:00:00:00:00:00", localMac)) {
                return localMac;
            }
            //第二种方式
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                ...
                while (networkInterfaces.hasMoreElements()) {
                    ...
                    if (null != networkInterface.getHardwareAddress() && StringUtil.isEquals("wlan0", networkInterface.getName())) {
                        localMac = displayMac(networkInterface.getHardwareAddress());
                        break;
                    }
                }
            }
            if (StringUtil.isNotEmpty(localMac) && StringUtil.isExistSubString(localMac, ":")) {
                return localMac;
            }
            //第三种方式获取
            localMac = getMacFromCallCmd();
            if (StringUtil.isNotEmpty(localMac)) {
                localMac = localMac.toLowerCase();
            }
...
//        DebugLog.v("tag", "cmd result : " + result);
    ...
    public static String displayMac(byte[] mac) {
        ...
        for (int i = 0; i < mac.length; i++) {
            ...
            if (StringUtil.isNotEmpty(macChar)) {
                if (1 == macChar.length()) {
                    macChar = "0" + macChar;
                }
            }
...
//                type = "无";
    ...
    private static int getNetworkClass() {
        ...
        try {
            final NetworkInfo network = ((ConnectivityManager) MyApplication.getInstance().getApplicationContext().getSystemService(Context.CONNECTIVITY_SERVICE)).getActiveNetworkInfo();
    ...
    public static String getImei() {
        try {
            TelephonyManager mTelephonyMgr = (TelephonyManager) MyApplication.getInstance().getApplicationContext().getSystemService(Context.TELEPHONY_SERVICE);
...
//        try {
//            TelephonyManager mTelephonyManager = (TelephonyManager) MyApplication.getInstance().getSystemService(Context.TELEPHONY_SERVICE);
