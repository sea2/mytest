...
public class PullToRefreshListView extends PullToRefreshBase<ListView> implements OnScrollListener {
     ...
     * @param isShowFootLoadEnabled scrollLoadEnabled=true,才会有用，不然没用，是否显示加载更多布局，可以不使用布局照样能加载更多
     */
    @Override
    public void setScrollLoadEnabled(boolean scrollLoadEnabled, boolean isShowFootLoadEnabled) {
        super.setScrollLoadEnabled(scrollLoadEnabled, isShowFootLoadEnabled);
        setPullLoadEnabled(false);
        if (scrollLoadEnabled && isShowFootLoadEnabled) {
            // 设置Footer
            if (null == mLoadMoreFooterLayout) {
                mLoadMoreFooterLayout = new FooterLoadingLayout(getContext());
            }

            if (mListView != null && mListView.getFooterViewsCount() == 0 && null == mLoadMoreFooterLayout.getParent()) {
    ...
    public FooterLoadingLayout getFooterLoading() {
        if (isScrollLoadEnabled()) {
            if (mLoadMoreFooterLayout != null)
                return (FooterLoadingLayout) mLoadMoreFooterLayout;
            else return null;
        } else return null;
    }
