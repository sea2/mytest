...
public class SwipeRefreshListView extends SwipeRefreshLayout {
    ...
    public SwipeRefreshListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        // 填充底部加载布局
        mFooterView = View.inflate(context, R.layout.list_view_footer, null);
    ...
    private boolean canLoadMore() {
        ...
        // 4. 当前页面可见的item是最后一个条目,一般最后一个条目位置需要大于第一页的数据长度
        boolean endItem = false;
        if (mListView != null && mListView.getAdapter() != null) {

            if (mItemCount > 0) {
                // 第一页未满，禁止下拉
                endItem = mListView.getAdapter().getCount() >= mItemCount && mListView.getLastVisiblePosition() == (mListView.getAdapter().getCount() - 1);
            } else {
                // 未设置数据长度，则默认第一页数据不满时也可以上拉
                endItem = mListView.getLastVisiblePosition() == (mListView.getAdapter().getCount() - 1);
            }
     ...
     * @param loadEnable
     */
    public void setLoadEnble(boolean loadEnable) {
        // 修改当前的状态
