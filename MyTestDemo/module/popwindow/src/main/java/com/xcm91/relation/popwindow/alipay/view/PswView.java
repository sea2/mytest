package com.xcm91.relation.popwindow.alipay.view;
...
public class PswView extends View {
    ...
    //边框宽度
    private int PwdTextView_border_width;
    ...
    public PswView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        showText = tArray.getBoolean(R.styleable.PwdTextView_showText, false);
        PwdTextView_border_width = tArray.getDimensionPixelSize(R.styleable.PwdTextView_border_width, 2);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPaint.setStrokeWidth(PwdTextView_border_width);
        mPaint.setStyle(Paint.Style.FILL);
        if (PwdTextView_border_width != 0) {
            //画边框
            mPaint.setStrokeWidth(PwdTextView_border_width);
        ...
        for (int i = 0; i < mPointNums; i++) {
            if (showText) {
                if (mCurrPsw.length() == mPointNums) {
                    String text = mCurrPsw.substring(i, i + 1);
                    mPaint.setTextSize(mPsw_size);
                    mPaint.setTextAlign(Paint.Align.CENTER);
                    Paint.FontMetricsInt fontMetrics = mPaint.getFontMetricsInt();
                    float fontHeight = fontMetrics.bottom - fontMetrics.top;
                    float baseY = getHeight() - (getHeight() - fontHeight) / 2 - fontMetrics.bottom;
                    canvas.drawText(text, w * (2 * i + 1), baseY - 2, mPaint);
                }
            } else canvas.drawCircle(w * (2 * i + 1), w, mPsw_size, mPaint);
        }
    }
