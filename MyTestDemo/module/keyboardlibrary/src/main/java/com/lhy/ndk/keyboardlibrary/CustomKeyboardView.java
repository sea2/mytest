package com.lhy.ndk.keyboardlibrary;
        ...
        try {   //凸显个别按键
            ...
            for (Keyboard.Key key : keys) {
               ...
               /* if (key.codes[0] == -5) {
                ...
                if (key.codes[0] == -1) {
                    Drawable dr = ContextCompat.getDrawable(context, R.drawable.keyboard_word_shift_layerlist);
                    Drawable dr_da = ContextCompat.getDrawable(context, R.drawable.keyboard_word_shift_layerlist_da);
                    dr.setBounds(key.x, key.y, key.x + key.width, key.y + key.height);
                    dr_da.setBounds(key.x, key.y, key.x + key.width, key.y + key.height);
